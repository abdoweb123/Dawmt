...
@section('content')
  <div class="container   text-center py-lg-5 py-2  rounded-3 position-relative container-policy">
    <div class="row justify-content-center position-relative py-lg-2 py-5">
      ...
      <div class="col-lg-10">
        <h4 class=" fw-bold h4-home">  @lang('trans.'.$type)</h4>
        <p>{{ setting($type.'Title_'.lang()) }}</p>
      </div>
    </div>
        ...
        <span class="bg-Secondary-color border border-2 border-black rounded-pill px-3 py-1 mx-4">@lang('trans.concerns')</span>
            ...
            <div class="position-absolute start-0 end-0 bottom-0 d-flex justify-content-between">
              <div class="arrow1 d-flex flex-column-reverse">
                <span class="bg-Secondary-color text-white border border-2 border-black rounded-pill mt-auto px-3 py-1 mx-4"> @lang('trans.'.$type)</span>
...
@push('scripts')
  <script>
    const columns = document.querySelectorAll('.sevices-fade div[data-aos="flip-up"]');
    let durationIncrement = 500;

    columns.forEach((col, index) => {
      let newDuration = 2000 - (durationIncrement * index);
